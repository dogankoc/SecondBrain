...
MACHINE_LINE_RE = re.compile(
    r"^(?:"
    r"\s*[\[{].*[\]}],?\s*|"
    r"\s*(?:INFO|DEBUG|TRACE|WARN|ERROR)\b.*|"
    r"\s*\d{4}-\d{2}-\d{2}[T ][0-9:.+-]+.*|"
    r"\s*[A-Za-z0-9+/]{180,}={0,2}\s*"
    r")$"
)
...
def _compact_fences(text: str) -> tuple[str, int]:
    ...
    def repl(match: re.Match[str]) -> str:
        ...
        head = body[:650].rstrip()
        tail = body[-250:].lstrip()
        label = lang or "text"
        return (
            f"```{label}\n{head}\n"
            f"\n[... {len(body) - 900} chars omitted by history prefilter ...]\n\n"
            f"{tail}\n```"
        )
...
def _trim_assistant(body: str, cap: int = 6500) -> str:
    ...
    for i in range(min(2, len(paras))):
        chosen.add(i)
    for i in range(max(0, len(paras) - 2), len(paras)):
        chosen.add(i)

    ranked = sorted(
        ((-_score_paragraph(p), i) for i, p in enumerate(paras)),
    ...
    for neg_score, i in ranked:
        if -neg_score <= 1:
            continue
        chosen.add(i)
        if sum(len(paras[j]) for j in chosen) >= cap * 0.9:
            break
    ...
    for i in sorted(chosen):
        p = paras[i]
        if used + len(p) > cap and result:
            continue
        if i > last + 1:
            result.append("[... low-signal assistant content omitted ...]")
        result.append(p)
        used += len(p)
        last = i
...
def _trim_user(body: str, cap: int = 9000) -> str:
    if len(body) <= cap:
        return body.strip()
    # User messages are the strongest evidence of goals/preferences, so keep both ends.
    head = body[:6500].rstrip()
    tail = body[-2000:].lstrip()
    return f"{head}\n\n[... {len(body) - 8500} chars omitted from very long user turn ...]\n\n{tail}"


def _speaker_from_block(block: str) -> str:
    first = block.splitlines()[0] if block.splitlines() else ""
    if first.startswith("### "):
        return first[4:].strip().lower()
    return ""


def prefilter_transcript(text: str) -> tuple[str, FilterStats]:
    original = text
    # Preserve frontmatter/title/metadata before the first conversation turn.
    ...
    for block in turn_blocks:
        ...
        body, n = _compact_fences(body)
        omitted_fenced += n
        body, n = _compact_machine_lines(body)
        omitted_machine += n

        speaker = _speaker_from_block(block)
        is_assistant = speaker in ASSISTANT_NAMES
        body = _trim_assistant(body) if is_assistant else _trim_user(body)
    ...
    stats = FilterStats(
        ...
        original_blocks=len(turn_blocks),
        kept_blocks=max(0, len(kept) - 1),
        omitted_fenced_chars=omitted_fenced,
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="Preview deterministic Second Brain history prefiltering.")
    ap.add_argument("file")
    ap.add_argument("--write", help="optional path for filtered preview")
    args = ap.parse_args()
    ...
    print(
        f"chars={stats.original_chars}->{stats.filtered_chars} "
        f"reduction={stats.reduction_percent:.1f}% "
        f"blocks={stats.original_blocks}->{stats.kept_blocks} "
        f"fenced_omitted={stats.omitted_fenced_chars} "
        f"machine_omitted={stats.omitted_machine_chars}"
    )
    if args.write:
        Path(args.write).write_text(filtered, encoding="utf-8")
