...
MACHINE_LINE_RE = re.compile(
    ...
    r"\s*\d{4}-\d{2}-\d{2}[T ][0-9:.+-]+.*|\s*[A-Za-z0-9+/]{120,}={0,2}\s*)$"
)
...
def _compact_machine_lines(text: str) -> tuple[str,int]:
    ...
    for line in text.splitlines():
        if len(line)>180 or MACHINE_LINE_RE.match(line):
            run.append(line)
        else:
...
def _score(p:str)->int:
    ...
    if any(x in low for x in ("error","exception","failed","success","complete","resolved","fixed")): score+=1
    if len(p)<=400: score+=1
    return score
...
def _select(body:str, cap:int, always_edges:int=1)->str:
    ...
    for i in sorted(chosen):
        p=paras[i]
        if i>last+1:
            out.append("[... low-signal content omitted ...]")
        out.append(p); last=i
    return "\n\n".join(out).strip()[:cap].rstrip()

def _trim_assistant(body:str, cap:int=1600)->str:
    # Historical assistant turns are the largest source of duplicate/explanatory bulk.
    if len(body)<=600:
        return body.strip()
    return _select(body,cap,always_edges=1)

def _trim_user(body:str, cap:int=4500)->str:
    # Preserve substantially more user text because it is strongest evidence of goals/preferences.
    if len(body)<=cap:
        return body.strip()
    head=body[:3200].rstrip(); tail=body[-900:].lstrip()
    return f"{head}\n\n[... {max(0,len(body)-4100)} chars omitted from very long user turn ...]\n\n{tail}"
